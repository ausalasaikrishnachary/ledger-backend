  ...
  db.getConnection((err, connection) => {
    ...
    connection.beginTransaction(async (err) => {
      ...
      try {
        ...
        // ===================================================
        // 🔥 INTEGRATION: TRIGGER RETAILER SCORE UPDATE
        // ===================================================
        try {
          // Use setTimeout to avoid blocking the response
          setTimeout(() => {
            // Update retailer score asynchronously
            updateRetailerScore(order.user_id, order.id)
              .then(result => {
                console.log('✅ Retailer score updated successfully:', result);
              })
              .catch(error => {
                console.warn('⚠️ Score update failed (non-critical):', error.message);
              });
          }, 1000); // 1 second delay
        } catch (scoreError) {
          console.warn('⚠️ Score update trigger failed (non-critical):', scoreError.message);
        }
        // ===================================================
...
// ===========================================================
// 🔥 HELPER FUNCTION: UPDATE RETAILER SCORE
// ===========================================================
function updateRetailerScore(customerId, orderId) {
  return new Promise((resolve, reject) => {
    if (!customerId) {
      return reject(new Error('customerId is required'));
    }

    // Create a new database connection for the score update
    db.getConnection((err, connection) => {
      if (err) {
        return reject(new Error('Database connection failed: ' + err.message));
      }

      // Import the scoring function (or define it inline)
      const calculateRetailerScore = async (connection, retailerId, period = '90') => {
        try {
      ...
      calculateRetailerScore(connection, customerId, '90')
        .then(result => {
          connection.release();
          resolve({
            success: true,
        ...
        .catch(error => {
          connection.release();
          console.warn('⚠️ Score calculation error:', error.message);
          // Still resolve so it doesn't fail the main order creation
          resolve({
            success: false,
            customerId: customerId,
            error: error.message,
