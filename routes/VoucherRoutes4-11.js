    ...
    connection.beginTransaction((err) => {
      ...
      processTransaction(transactionData, 'Sales', connection)
        .then((result) => {
          ...
          const voucherID = result.voucherId || result.insertId; // Use the created voucher ID
          const date = transactionData.invoiceDate || new Date();
          const transactionType = transactionData.type || 'Sales';
          const accountID = transactionData.selectedSupplierId; // Using selectedSupplierId as AccountID
          ...
          console.log('Ledger data prepared:', {
            voucherID, date, transactionType, accountID, accountName, amount
          });

          // Validate required fields
          if (!voucherID || !accountID || isNaN(parseFloat(amount))) {
          ...
          connection.query(getBalanceQuery, [accountID], (balanceErr, balanceResults) => {
            ...
            console.log('Executing ledger insert with values:', ledgerValues);

            connection.query(ledgerQuery, ledgerValues, (ledgerErr, ledgerResults) => {
              if (ledgerErr) {
                console.error('Ledger insert error:', ledgerErr);
                return connection.rollback(() => {
                  connection.release();
                  res.status(500).send({ error: 'Failed to insert ledger entry', details: ledgerErr.message });
                });
              }
              ...
              connection.commit((commitErr) => {
                ...
                connection.release();
                console.log('Sales transaction and ledger entry completed successfully');
                res.send({
