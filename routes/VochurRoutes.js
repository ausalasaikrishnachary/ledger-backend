      ...
      try {
        ...
        for (const item of originalBatchDetails) {
          ...
          if (originalTransactionType === "Purchase" || originalTransactionType === "CreditNote") {
            ...
            if (r.affectedRows === 0) {
              throw new Error(
                `Failed to reverse ${originalTransactionType} for batch ${item.batch}`
              );
            }
            ...
            // SALES reversal → return stock (no quantity check needed as we're adding stock)
            const q = `
              UPDATE batches
              SET quantity = quantity + ?,
                  stock_out = stock_out - ?,
        ...
        // REMOVED BatchDetails from the UPDATE statement
        await queryPromise(
          connection,
          `UPDATE voucher 
        ...
        // -------------------------------------------------------------------
        // INSERT NEW voucherDetails (FIXED: INCLUDED InvoiceNumber)
        // -------------------------------------------------------------------
        for (const item of newBatchDetails) {
          await queryPromise(
            connection,
        ...
        for (const item of newBatchDetails) {
          ...
          const batchExists = await queryPromise(
            connection,
            "SELECT 1 FROM batches WHERE product_id = ? AND batch_number = ?",
            [item.product_id, item.batch]
          );

          if (batchExists.length === 0) {
            throw new Error(`Batch ${item.batch} not found for product ${item.product_id}`);
          }
          ...
          if (originalTransactionType === "Purchase" || originalTransactionType === "CreditNote") {
            ...
            const availableQuantity = parseFloat(currentBatch[0].quantity);
            if (availableQuantity < item.quantity) {
              throw new Error(
                `Insufficient quantity for SALES update in batch ${item.batch}. Available: ${availableQuantity}, Required: ${item.quantity}`
              );
            }
